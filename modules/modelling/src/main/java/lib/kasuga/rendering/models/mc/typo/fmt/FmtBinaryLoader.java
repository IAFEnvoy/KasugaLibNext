...
public final class FmtBinaryLoader implements ModelLoader<byte[], ResourceLocation, Integer> {
    ...
    private static void addQuad(Quad quad, Material material, Bone root, List<Vertex> vertices, List<Mesh> meshes) {
        Vector3f[] p = quad.positions(); Vector3f normal = new Vector3f(p[1]).sub(p[0]).cross(new Vector3f(p[2]).sub(p[0])).normalize();
        Mesh mesh = new Mesh(new Vertex[p.length], normal, new Transform(), new Material[]{material}, null);
        for (int i=0;i<p.length;i++) { Vertex v = new Vertex(p[i], null); v.addUV(mesh, material, quad.uvs()[i]); v.setBinding(new BoneBinding(new Pair[]{Pair.of(root, 1f)}, BoneBindingFunc.BDEF, null)); mesh.getVertices()[i] = v; vertices.add(v); }
        meshes.add(mesh);
    }
    ...
    static final class BobRecords {
        static List<Quad> read(byte[] data) {
            ...
            while (b.hasRemaining()) { int code = b.get() & 255;
                try { switch (code) {
                    case 0 -> object = false;
                    case 1 -> readString(b);
                    case 2 -> { if (object) skip(b, 12); else readString(b); }
                    case 3 -> { if (object) skip(b, 12); else skip(b, 8); }
                    case 4 -> { if (object) vertices.add(new Vector3f(b.getFloat(), b.getFloat(), b.getFloat())); else readString(b); }
                    case 5 -> { if (object) uvs.add(new Vector2f(b.getFloat(), b.getFloat())); else object = true; }
                    case 6 -> skip(b, 12);
                    case 7 -> { int count = b.getInt(); if (count < 3 || count > 64 || b.remaining() < count * 8) break; Vector3f[] p = new Vector3f[count]; Vector2f[] uv = new Vector2f[count]; for (int i=0;i<count;i++) { int vi=b.getInt(), ui=b.getInt(); if (vi < 0 || ui < 0 || vi >= vertices.size() || ui >= uvs.size()) { p=null; break; } p[i]=vertices.get(vi); uv[i]=uvs.get(ui); } if (p != null && p.length == 4) out.add(new Quad(p, uv)); }
                    default -> { if (code == 4 && b.remaining() >= 12) { vertices.add(new Vector3f(b.getFloat(), b.getFloat(), b.getFloat())); } }
                } } catch (RuntimeException e) { break; }
                // Codes 4/5 are ambiguous until their payload length is known; recover vector/UV streams from headers.
            }
            return out;
        }
    }
