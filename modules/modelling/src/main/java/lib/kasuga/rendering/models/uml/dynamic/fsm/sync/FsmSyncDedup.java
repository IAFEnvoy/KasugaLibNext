...
/**
 * Per-{@code (key, player)} version dedup and forced-heartbeat cadence for {@link FsmSyncServer}.
...
final class FsmSyncDedup {
    ...
    boolean isHeartbeatTick(FsmSyncKey key) {
    ...
    boolean shouldSend(FsmSyncKey key, UUID playerId, int version, boolean heartbeat) {
        if (heartbeat) {
    ...
    void recordSent(FsmSyncKey key, UUID playerId, int version) {
